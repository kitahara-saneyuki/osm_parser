...
def import_edges():
    ...
    run_shell_command(
        "dags/01_parse_map/sql/01c_parse_osm/07_import_edges.sh {db_pgpass} data/{atlas_region}/temp \
...
with DAG(
    ...
    clean_all_roads = PostgresOperator(
        ...
        sql="sql/01c_parse_osm/01_clean_all_roads.sql",
    )
    ...
    modeling_nodes = PostgresOperator(
        ...
        postgres_conn_id="{{ dag_run.conf['atlas_region'] }}",
        sql="sql/01c_parse_osm/03_modeling_nodes.sql",
    )

    routing_nodes = PostgresOperator(
        task_id="04_routing_nodes",
        postgres_conn_id="{{ dag_run.conf['atlas_region'] }}",
        sql="sql/01c_parse_osm/04_routing_nodes.sql",
    )

    new_edge_table = PostgresOperator(
        task_id="05_new_edge_table",
        postgres_conn_id="{{ dag_run.conf['atlas_region'] }}",
        sql="sql/01c_parse_osm/05_new_edge_table.sql",
    )

    post_processing = PostgresOperator(
        task_id="08_post_processing",
        postgres_conn_id="{{ dag_run.conf['atlas_region'] }}",
        sql="sql/01c_parse_osm/08_post_processing.sql",
    )
